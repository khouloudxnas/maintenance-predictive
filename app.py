...
# ===================== UPLOAD & ANALYSE =====================
    ...
    with tab1:
        ...
        if uploaded_file is not None:
            try:
                ...
                col4.metric("💾 Taille", f"{uploaded_file.size / 1024:.1f} KB")
    ...
    with tab2:
        ...
        if uploaded_model is not None:
            try:
                ext = uploaded_model.name.split(".")[-1]
                ...
                st.success(f"✅ Modèle chargé : {uploaded_model.name}")
    ...
    with tab3:
        ...
        if numeric_cols:
            col1, col2 = st.columns(2)
            selected_feature = col1.selectbox("Variable à visualiser", numeric_cols)
            chart_type = col2.selectbox("Type de graphique", ["Ligne", "Histogramme", "Box Plot"])

            if chart_type == "Ligne":
                fig = px.line(df, y=selected_feature, title=f"Évolution de {selected_feature}")
            elif chart_type == "Histogramme":
                fig = px.histogram(df, x=selected_feature, nbins=30, title=f"Distribution de {selected_feature}")
            else:
                fig = px.box(df, y=selected_feature, title=f"Box Plot de {selected_feature}")
        ...
        with col1:
            nb_pannes = st.number_input("Nombre de pannes", min_value=1, value=5)
            temps_fonctionnement = st.number_input("Temps total de fonctionnement (h)", min_value=1, value=500)
            temps_reparation = st.number_input("Temps total de réparation (h)", min_value=0, value=25)

        if col2.button("🔢 Calculer les KPIs"):
            mtbf = temps_fonctionnement / nb_pannes
            mttr = temps_reparation / nb_pannes
            disponibilite = (mtbf / (mtbf + mttr)) * 100

            col_a, col_b, col_c = st.columns(3)
            col_a.metric("⏱️ MTBF", f"{mtbf:.1f} h")
            col_b.metric("🔧 MTTR", f"{mttr:.1f} h")
            col_c.metric("⚙️ Disponibilité", f"{disponibilite:.1f} %")
...
st.markdown("---")
        ...
        if numeric_cols:
            anomaly_feature = st.selectbox("Variable à analyser", numeric_cols)
            threshold = st.slider("Seuil (écart-type)", 1.0, 5.0, 3.0)
            ...
            if st.button("🔍 Détecter les anomalies"):
                ...
                fig.add_trace(go.Scatter(
                    x=df.index,
                    y=df[anomaly_feature],
                    mode='markers',
                    marker=dict(color=np.where(df['Anomaly'], "red", "blue"), size=6),
                    name="Données"
                ))
                fig.add_hline(y=mean + threshold * std, line_dash="dash", line_color="red")
                fig.add_hline(y=mean - threshold * std, line_dash="dash", line_color="red")

                fig.update_layout(title=f"Détection d'anomalies : {anomaly_feature}",
                                  xaxis_title="Index", yaxis_title=anomaly_feature, height=400)
